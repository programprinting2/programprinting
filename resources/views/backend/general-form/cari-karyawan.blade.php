@php
  $modalId = $modalId ?? 'modalCariKaryawan';
  $inputId = $inputId ?? 'searchKaryawan';
  $tableId = $tableId ?? 'tabelCariKaryawan';
  $paginationId = $paginationId ?? 'paginationCariKaryawan';
  $clearBtnId = $clearBtnId ?? 'clearSearchKaryawan';
@endphp
  ...
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content border border-primary">
      <div class="modal-header">
        <h5 class="modal-title" id="{{ $modalId }}Label">Cari Karyawan</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <div class="mb-3">
          <label for="{{ $inputId }}" class="form-label">Cari Karyawan</label>
...
(function() {
  ...
  $(document).on('shown.bs.modal', '#' + modalId, function () {
    loadKaryawan($('#' + inputId).val(), 1);
  });
  $(document).on('keypress', '#' + inputId, function(event) {
    if (event.key === 'Enter') {
      loadKaryawan(this.value, 1);
  ...
  $(document).on('click', '#' + paginationId + ' .page-link', function(e) {
    ...
    loadKaryawan(search, page);
  });
  $(document).on('click', '#' + tableId + ' .pilih-karyawan', function() {
    const data = {
      id: $(this).data('id'),
      nama: $(this).data('nama'),
      kode: $(this).data('kode'),
    ...
    window.dispatchEvent(new CustomEvent('karyawanDipilih', { detail: data }));
    $('#' + modalId).modal('hide');
  });
})();
</script>

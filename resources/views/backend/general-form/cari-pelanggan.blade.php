@php
  $modalId = $modalId ?? 'modalCariPelanggan';
  $inputId = $inputId ?? 'searchPelanggan';
  $tableId = $tableId ?? 'tabelCariPelanggan';
  $paginationId = $paginationId ?? 'paginationCariPelanggan';
  $clearBtnId = $clearBtnId ?? 'clearSearchPelanggan';
@endphp
  ...
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content border border-primary">
      <div class="modal-header">
        <h5 class="modal-title" id="{{ $modalId }}Label">Cari Pelanggan</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <div class="mb-3">
          <label for="{{ $inputId }}" class="form-label">Cari Pelanggan</label>
          <div class="input-group">
            <span class="input-group-text bg-light"><i data-feather="search" class="icon-sm"></i></span>
            <input type="text" class="form-control" id="{{ $inputId }}" placeholder="Cari berdasarkan nama, kode, handphone, email...">
...
(function() {
  ...
  $(document).on('shown.bs.modal', '#' + modalId, function () {
    loadPelanggan($('#' + inputId).val(), 1);
  });
  $(document).on('keypress', '#' + inputId, function(event) {
    if (event.key === 'Enter') {
      loadPelanggan(this.value, 1);
  ...
  $(document).on('click', '#' + paginationId + ' .page-link', function(e) {
    ...
    loadPelanggan(search, page);
  });
  $(document).on('click', '#' + tableId + ' .pilih-pelanggan', function() {
    const data = {
      id: $(this).data('id'),
      nama: $(this).data('nama'),
      kode: $(this).data('kode'),
    ...
    window.dispatchEvent(new CustomEvent('pelangganDipilih', { detail: data }));
    $('#' + modalId).modal('hide');
  });
})();
</script>

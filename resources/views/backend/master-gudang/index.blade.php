...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <div>
                    <h6 class="card-title mb-0">Master Gudang</h6>
                    <p class="text-muted">Manajemen gudang penyimpanan</p>
                </div>
                <div>
                    <button type="button" class="btn btn-primary d-flex align-items-center" data-bs-toggle="modal" data-bs-target="#tambahGudang">
                        <i class="link-icon icon-sm me-1" data-feather="plus"></i> Tambah Gudang
                    </button>
                </div>
            </div>
            <form class="row g-3 mb-3" method="GET" action="{{ route('backend.master-gudang.index') }}">
                <div class="col-md-6">
                    <div class="input-group">
                        <span class="input-group-text bg-light"><i data-feather="search" class="icon-sm"></i></span>
                        <input type="text" class="form-control" name="search" placeholder="Cari gudang..." value="{{ request('search') }}">
                    </div>
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    <a href="{{ route('backend.master-gudang.index') }}" class="btn btn-outline-secondary w-100">
            ...
            <div class="table-responsive">
                        ...
                        @forelse ($gudang as $item)
                            <tr>
                                ...
                                <td>
                                    <div>
                                        {{ $item->alamat }}<br>
                                        <small class="text-muted">{{ $item->kota }}, {{ $item->provinsi }}</small>
                                    </div>
                                </td>
                                <td>
                                    <b>{{ $item->kapasitas }}</b><br>
                                </td>
                                <td>{{ $item->rak_count }}</td>
                                <td>
                                    <div>
                                        {{ $item->manager }}<br>
                                        @if($item->email)
                                ...
                                <td>
                                    <div class="btn-group gap-1" role="group">
                                        <button type="button" class="btn btn-warning btn-xs rounded" title="Edit" 
                                            ...
                                            data-gudang='@json($item)'>
                                            <i class="link-icon icon-sm" data-feather="edit"></i>
                                        </button>
                                        <button type="button" class="btn btn-danger btn-xs rounded btn-delete-gudang" 
                                                data-id="{{ $item->id }}" 
                                                data-nama="{{ $item->nama_gudang }}" 
                                                title="Hapus">
                                            <i class="link-icon icon-sm" data-feather="trash"></i>
                                        </button>
                                        <form id="formDeleteGudang{{ $item->id }}" 
                                              action="{{ route('backend.master-gudang.destroy', $item->id) }}" 
                ...
                <div class="d-flex justify-content-between align-items-center mt-3">
                    <p class="text-muted">
                        Menampilkan {{ $gudang->firstItem() ?? 0 }} - {{ $gudang->lastItem() ?? 0 }} dari {{ $gudang->total() }} gudang
                    </p>
                    <div>
                        {{ $gudang->appends(request()->query())->links('pagination::bootstrap-4') }}
                    </div>
                </div>
            </div>
        </div>
    ...
    @include('backend.master-gudang.modal_form')
    @include('backend.master-gudang.modal_edit')
@endsection
...
@push('custom-scripts')
    ...
    <script src="{{ asset('js/master-gudang/edit-modal.js') }}"></script>
    <script src="{{ asset('js/master-gudang/add-modal.js') }}"></script>
    <script>
        feather.replace();
        
        // Delete confirmation
        $('.btn-delete-gudang').click(function (e) {
            e.preventDefault();
            var id = $(this).data('id');
            var nama = $(this).data('nama');
            Swal.fire({
                title: 'Hapus Data Gudang?',
                html: `Anda akan menghapus data gudang <b>"${nama}"</b>.<br><span class='text-danger fw-bold'>Seluruh rak di dalam gudang ini juga akan ikut terhapus secara permanen!</span><br>Data yang sudah dihapus tidak dapat dikembalikan.`,
                icon: 'warning',

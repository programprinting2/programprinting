      ...
      <div class="tab-pane fade show active" id="detailPesanan" role="tabpanel">
        <div class="card mb-3">
          <div class="card-body">
            <div class="row">
              <div class="col-md-6">
                <div class="mb-2"><span class="text-muted">Kode Pembelian</span><br><span class="fw-semibold">{{ $pembelian->kode_pembelian }}</span></div>
                <div class="mb-2"><span class="text-muted">Tanggal Pesanan</span><br><span class="fw-semibold">{{ \Carbon\Carbon::parse($pembelian->tanggal_pembelian)->locale('id')->translatedFormat('d F Y') }}</span></div>
                <div class="mb-2"><span class="text-muted">Tanggal Jatuh Tempo</span><br><span class="fw-semibold">{{ $pembelian->jatuh_tempo ? \Carbon\Carbon::parse($pembelian->jatuh_tempo)->locale('id')->translatedFormat('d F Y') : '-' }}</span></div>
                <div class="mb-2"><span class="text-muted">Nomor Form</span><br><span class="fw-semibold">{{ $pembelian->nomor_form ?? '-' }}</span></div>
              </div>
              <div class="col-md-6">
                <div class="mb-2"><span class="text-muted">Diskon (%)</span><br><span class="fw-semibold">{{ $pembelian->diskon_persen ?? '-' }}%</span></div>
                <div class="mb-2"><span class="text-muted">Catatan</span><br><span class="fw-semibold">{{ $pembelian->catatan ?? '-' }}</span></div>
                <div class="mb-2"><span class="text-muted">Total Pembelian</span><br><span class="fw-semibold text-success">Rp {{ number_format($pembelian->total,0,',','.') }}</span></div>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="card">
      <div class="card-body">
        ...
        <div class="d-flex justify-content-between mb-1 text-danger">
          ...
          <span>- Rp {{ number_format($pembelian->items->sum('subtotal') * ($pembelian->diskon_persen/100),0,',','.') }}</span>
        </div>
        @endif
        ...
        <div class="d-flex justify-content-between mb-1">
          ...
          <span>Rp {{ number_format(($pembelian->items->sum('subtotal') * (1 - ($pembelian->diskon_persen/100))) * ($pembelian->tarif_pajak/100)),0,',','.' }}</span>
        </div>
        @endif

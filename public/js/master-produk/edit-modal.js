$(function () {
    ...
    $(document).on("click", ".btn-edit-produk", function () {
        ...
        $.get("/backend/master-produk/" + id + "/edit", function (res) {
            if (res.success) {
                ...
                window.parameterMesinListFromBackend = p.parameter_modal_json || [];
    ...
    function renderDocumentsPreview() {
        ...
        existingDocuments.forEach((doc, idx) => {
          ...
          <tr>
            ...
            <td>${doc.jenis || "-"}</td>
            <td>${
                doc.ukuran ? (doc.ukuran / 1024).toFixed(1) + " KB" : "-"
            }</td>
    ...
    function convertSavedParameterToInternal(savedData) {
        ...
        return savedData.map(savedItem => {
            // Cari mesin berdasarkan mesin_id
            const mesin = window.masterMesinList?.find(m => m.id == savedItem.mesin_id);
            if (!mesin) {
                console.warn(`Mesin dengan ID ${savedItem.mesin_id} tidak ditemukan`);
                return null;
            }
            ...
            if (!profil) {
                console.warn(`Mesin ${mesin.nama_mesin} tidak memiliki parameter biaya`);
                return null;
            }
            ...
            const selectedIndex = profil.findIndex(p => p.nama === savedItem.nama_parameter);
            
            if (selectedIndex === -1) {
                console.warn(`Parameter "${savedItem.nama_parameter}" tidak ditemukan di mesin ${mesin.nama_mesin}`);
                return null;
            }
    ...
    $("#editProdukModal").on("shown.bs.modal", function () {
        ...
        if (Array.isArray(produkData) && produkData.length > 0) {
            editParameterMesinList = convertSavedParameterToInternal(produkData);
        } else {
    ...
    $(document).on("click", "#editBtnTambahParameter", function (e) {
        e.preventDefault();
    ...
    // Handler mesinDipilih di modal edit
    window.addEventListener("mesinDipilih", function (e) {
        // context untuk edit: pastikan ini popup dari modal edit!
        if (!$("#modalCariMesinProdukEdit").hasClass("show")) return;
        const data = e.detail;
    ...
    function renderEditParameterMesinTable() {
        ...
        <tr data-mesin-id="${row.mesin_id}">
          ...
          <td><input type="number" class="form-control form-control-sm edit-jumlah-param" min="1" value="${
              row.jumlah
          }" data-idx="${idx}"></td>
      ...
      <div class="border rounded mb-3 p-3 position-relative mesin-item" data-index="${index}">
        ...
        <div class="row mb-2">
          <div class="col-md-6">
            ...
            <small class="text-muted">Tipe: <span>${
